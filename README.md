...
- **`--file` / stdin** — Feed file content directly into the prompt
- **`--batch` Mode** — Process multiple files in one command
- **`--cache` Mode** — Skip re-processing unchanged files
- **`--role` Presets** — Named role configs that bind task type to model (e.g. `summarize` → 4B, `code` → 30B, `deepseek` → cloud)
...
```bash
# Build
git clone https://github.com/go7th/gowork.git
cd gowork
cargo build --release
...
# Use a role preset (routes to different model per task type)
gowork --role summarize --file big.log -p "extract errors"
gowork --role code --file main.rs -p "review this module"
gowork --role deepseek -p "translate this to English"
...
# Role presets — pick the right model for each task type
[roles.summarize]
description = "Fast summarize / file cleaning (Claude preprocessing)"
model = "mlx-community/Qwen3.5-4B-OptiQ-4bit"
no_tools = true              # This role defaults to --no-tools

[roles.code]
description = "Code review, refactoring analysis (MoE, quality-first)"
model = "mlx-community/Qwen3-Coder-30B-A3B-Instruct-4bit"

[roles.deepseek]
description = "Cloud fallback (when local GPU is busy)"
...
Environment variables: `GOWORK_BASE_URL`, `GOWORK_MODEL`, `GOWORK_API_KEY`, `GOWORK_ROLE`

**Priority:** CLI flags > role override > env vars > top-level config > built-in defaults.
...
| `--role <name>` | Use a named role preset from config (e.g. `summarize`, `code`) |
| `--list-roles` | List all configured roles and exit |
...
- **`--file` / 管道** -- 文件内容直接拼到 prompt
- **`--batch` 批量处理** -- 一条命令处理多个文件
- **`--cache` 缓存** -- 文件没改就不重复处理
- **`--role` 角色预设** -- 把任务类型绑定到模型（如 `summarize`→4B、`code`→30B、`deepseek`→云端）
...
# 使用角色预设（按任务类型自动选模型）
gowork --role summarize --file big.log -p "提取错误"
gowork --role code --file main.rs -p "审查这个模块"
gowork --role deepseek -p "翻译成英文"
...
# 角色预设 —— 不同任务用不同模型
[roles.summarize]
description = "快速摘要、文件清洗（给 Claude 做预处理）"
model = "mlx-community/Qwen3.5-4B-OptiQ-4bit"
no_tools = true          # 这个角色默认开 --no-tools

[roles.code]
description = "代码审查、重构分析（MoE 大模型，质量优先）"
model = "mlx-community/Qwen3-Coder-30B-A3B-Instruct-4bit"

[roles.deepseek]
description = "云端兜底（本地 GPU 忙的时候用）"
base_url = "https://api.deepseek.com/v1"
model = "deepseek-chat"
api_key = "sk-xxx"
no_tools = true
...
环境变量：`GOWORK_BASE_URL`、`GOWORK_MODEL`、`GOWORK_API_KEY`、`GOWORK_ROLE`

**优先级：** 命令行参数 > 角色配置 > 环境变量 > 顶层配置 > 默认值
...
- **`--file` / 標準入力** -- ファイル内容をプロンプトに直接注入
- **`--batch` モード** -- 1コマンドで複数ファイルを処理
- **`--cache` モード** -- 未変更ファイルの再処理をスキップ
- **`--role` ロールプリセット** -- タスクタイプごとにモデルを紐付け（`summarize`→4B、`code`→30B、`deepseek`→クラウド）
...
# ロールプリセット（タスク種別ごとに自動でモデル切替）
gowork --role summarize --file big.log -p "エラーを抽出"
gowork --role code --file main.rs -p "このモジュールをレビュー"
gowork --role deepseek -p "英語に翻訳"
...
# ロールプリセット —— タスクごとに最適なモデルを選ぶ
[roles.summarize]
description = "高速要約 / ファイルクレンジング（Claude 前処理用）"
model = "mlx-community/Qwen3.5-4B-OptiQ-4bit"
no_tools = true

[roles.code]
description = "コードレビュー、リファクタ分析（MoE、品質優先）"
model = "mlx-community/Qwen3-Coder-30B-A3B-Instruct-4bit"

[roles.deepseek]
description = "クラウドフォールバック（ローカル GPU が忙しい時）"
base_url = "https://api.deepseek.com/v1"
model = "deepseek-chat"
api_key = "sk-xxx"
no_tools = true
...
環境変数：`GOWORK_BASE_URL`、`GOWORK_MODEL`、`GOWORK_API_KEY`、`GOWORK_ROLE`

**優先順位：** CLI フラグ > ロール設定 > 環境変数 > トップレベル設定 > デフォルト値
...
获取更多 AI 工具、本地大模型实践、开发技巧分享：

<div align="center">
<img src="assets/wechat-qrcode.jpg" alt="微信公众号：浅思集录" width="200">
...
[Report Bug](https://github.com/go7th/gowork/issues) | [Request Feature](https://github.com/go7th/gowork/issues)
